...
const Depuration = () => {
  ...
  const handleEdit = (id) => {
    navigate(`/admin/personalize_editor/edit/${id}`);
  };
                ...
                <tbody>
                  {currentItems.map((tema) => (
                    <tr key={tema.Tema_Juego_ID_PK}>
                      <td style={{ display: 'none' }}>{tema.Tema_Juego_ID_PK}</td> {/* Ocultar el ID */}
                      ...
                      <td className="personalization__actions">
                        {/* Solo mostrar el botón de editar si no es un Rompecabezas */}
                        {tema.Tipo_Juego !== "Rompecabezas" && (
                          <button 
                            onClick={() => handleEdit(tema.Tema_Juego_ID_PK)} 
                        ...
                        <button 
                          onClick={() => handleDelete(tema.Tema_Juego_ID_PK)} 
                          style={{ cursor: "pointer" }}
                        >
                          ❌
                        </button>
